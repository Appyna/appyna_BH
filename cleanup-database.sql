...
-- ÉTAPE 1: Identifier l'admin (email: appyna.contact@gmail.com)
...
WHERE conversation_id IN (
  ...
  WHERE user1_id NOT IN (
    SELECT id FROM users WHERE email = 'appyna.contact@gmail.com'
  )
  OR user2_id NOT IN (
    SELECT id FROM users WHERE email = 'appyna.contact@gmail.com'
  )
...
DELETE FROM users
WHERE email != 'appyna.contact@gmail.com';
